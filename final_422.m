...
xe = 1.8; %Speed at which habitat moves for each timestep
...
new_R_vec = R_scales.*init_R_vec;
new_R_vec = new_R_vec * (trapz(init_R_vec)/trapz(new_R_vec));
%cap R values so they dont get unrealistic
new_R_vec(new_R_vec>2*R) = 2*R;


%RUN DETERMINISTIC MODEL
burn = 5; %Number of iterations to get equilibrium before adding climate change
param = 1;
for i = 1:burn
